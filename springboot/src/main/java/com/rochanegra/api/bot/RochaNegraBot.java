...
public class RochaNegraBot extends AbilityBot implements SpringLongPollingBot {
    ...
    public Reply replyToText() {
        BiConsumer<BaseAbilityBot, Update> action = (bot, upd) -> silent.send("Echo: " + upd.getMessage().getText(),
                upd.getMessage().getChatId());

        return Reply.of(action, Flag.TEXT, upd -> !upd.getMessage().getText().startsWith("/"));
    }
    ...
    public Reply replyToMedia() {
        BiConsumer<BaseAbilityBot, Update> action = (bot, upd) -> {
            ...
            if (upd.getMessage().hasPhoto()) {
                silent.send("I received a photo! In the future, I'll scan this for receipts. 📸", chatId);
            } else if (upd.getMessage().hasDocument()) {
                silent.send("I received a document! I can use this to import bank statements soon. 📄", chatId);
            } else if (upd.getMessage().hasAudio() || upd.getMessage().hasVoice()) {
                silent.send("I received audio! Maybe one day I'll be able to transcribe your voice notes. 🎙️", chatId);
            } else if (upd.getMessage().hasSticker()) {
                silent.send("Nice sticker! I like your style. 🎨", chatId);
            } else {
                silent.send("I received some media, but I'm not sure how to handle it yet!", chatId);
            }
        };
