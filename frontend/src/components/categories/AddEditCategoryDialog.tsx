...
export function AddEditCategoryDialog({ isOpen, onOpenChange, editingCategory, onAddCategory, onUpdateCategory }: AddEditCategoryDialogProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingCategory) {
            onUpdateCategory(editingCategory.id, { name, iconSlug: editingCategory.iconSlug, color });
        } else {
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 pt-4">
                    ...
                    <div className="space-y-2">
                        <Label>Icon</Label>
                        <div className="flex flex-wrap gap-2 h-32 overflow-y-auto p-2 border rounded-md">
                            {ICONS.map(iconName => {
